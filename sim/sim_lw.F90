...
contains
   ...
   subroutine lw_stream(nx,ny,fsrc,fdst,dt)
      ...
      !$omp parallel do simd collapse(3) schedule(simd: static) nontemporal(fdst) &
...
module sim_lw_class
   ...
   use lbm_lw, only: lw_collision, lw_stream, lw_bc
   implicit none
   ...
   subroutine lw_step(this,omega)
      ...
      associate( &
         ...
         case(0)
            ...
            call lw_bc(nx,ny,grid%f2)
         case(1)
            call lw_stream(nx,ny,grid%f2,grid%f1,this%dt)
            call lw_collision(nx,ny,grid%f1,omega)
            call lw_bc(nx,ny,grid%f1)
         end select
...
function c_lw_init(nx,ny,dt,rho,u,sigma,params) bind(c)
...
subroutine c_lw_step(ptr,omega) bind(c)
...
subroutine c_lw_vars(ptr,rho,u) bind(c)
   use, intrinsic :: iso_c_binding, only: c_ptr, c_double, c_f_pointer
   use sim_lw_class
   type(c_ptr), value :: ptr
...
subroutine c_lw_free(ptr) bind(c)
...
function c_lw_norm(nx,ny,u,ua) bind(c)
   use, intrinsic :: iso_c_binding, only: c_int, c_double
   use sim_lw_class
   integer(c_int), value :: nx, ny
   real(c_double), intent(in) :: u(nx,ny), ua(nx,ny)
   real(c_double) :: c_lw_norm
   c_lw_norm = norm2(u - ua) / norm2(ua)
end function
